...
extern int MainPatch()
{
     DEBUG_FUNCTION_LINE("Trying to IOSUHAX");
    DEBUG_FUNCTION_LINE("ISOUHAX VALUE %i",IOSUHAX_Open(NULL));
    if (IOSUHAX_Open(NULL) >= 0) {
        /* NoSSL patch */
        DEBUG_FUNCTION_LINE("Doing NoSLL Patch");
        IOSUHAX_kern_write32(0xE1019F78,0xE3A00001); // mov r0, #1 ,sizeof(0xE3A00001)

        /* URL patch */
        DEBUG_FUNCTION_LINE("Patching URLS");
        for(int i = 0; i < sizeof(url_patches)/sizeof(URL_Patch); i++) {
            write_string(url_patches[i].address, url_patches[i].url);
        }

        IOSUHAX_Close();
        DEBUG_FUNCTION_LINE("DONE IOSUHAX");
    }
