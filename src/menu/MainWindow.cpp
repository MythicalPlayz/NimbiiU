...
MainWindow::MainWindow(int32_t w, int32_t h, Renderer* renderer) : GuiFrame(w, h)  {
  ...
  //  label = new GuiText("Exit Installer", {255, 255, 0, 255}, fc_font);
...
*//*
  ...
  //  this->append(image3);
   ...
   // image4->setAlignment(ALIGN_BOTTOM | ALIGN_RIGHT);
    ...
    auto postionx = label->getOffsetX();
    auto postiony = label->getOffsetX();
    label->setPosition(postionx,postiony + 50);
